          ...
          return (
            <Card key={key} size="sm">
              <CardContent className="space-y-2">
                ...
                <p className="text-[11px] text-muted-foreground">{Math.round(category.weight * 100)}% of overall score</p>
              </CardContent>
            </Card>
          );
        })}
      ...
      <details className="group rounded-lg border border-border bg-card text-sm">
        ...
        <div className="space-y-4 border-t border-border px-4 py-4">
          <p className="text-xs text-muted-foreground">
            Overall Score = Market × 20% + Unit Economics × 20% + Financial Viability × 20% + Validation × 15% + Execution × 15% + Risk × 10%.
            Every category score is 0-100, derived transparently from your inputs and business-model-specific benchmarks — nothing here is a guess.
          </p>
          {CATEGORY_ORDER.map((key) => {
            const category = scores.categories[key];
            return (
