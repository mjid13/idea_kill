  ...
  return (
    <div className="mx-auto max-w-3xl px-6 py-10 print:max-w-none print:px-0">
      ...
      <header className="mb-8 border-b border-border pb-6">
        <p className="text-xs font-medium tracking-wide text-muted-foreground uppercase">Product Viability Report</p>
        <h1 className="mt-1 text-3xl font-semibold text-foreground">{project.basicInfo.name || "Untitled project"}</h1>
        <p className="mt-1 text-sm text-muted-foreground">
          {BUSINESS_MODEL_LABELS[project.basicInfo.businessModel]} · {project.basicInfo.currency} · Generated {new Date().toLocaleDateString()}
        </p>
      ...
      <Section title="Viability score">
        <div className="flex items-baseline gap-6">
          <div>
            ...
            <p className="text-sm text-muted-foreground">{classification} — {CLASSIFICATION_DESCRIPTIONS[classification]}</p>
          </div>
          <div>
          ...
          <div>
            <p className="text-lg font-semibold text-foreground">Stage {scores.maturityStage.stage}</p>
            <p className="text-xs text-muted-foreground">{scores.maturityStage.label}</p>
          </div>
        </div>
      ...
      <Section title="Financial viability">
        <KeyValueGrid
          items={[
            ["Monthly burn", metrics.operating.isCashFlowPositive ? "Cash Flow Positive" : formatCurrency(metrics.operating.monthlyBurn, currency, { compact: true })],
            ["Runway", metrics.funding.isProfitable ? "Profitable / no finite runway" : formatMonths(metrics.funding.runwayMonths)],
            [
              "Break-even customers",
              metrics.breakEven.breakEvenCustomers === null ? "Unreachable at current margins" : metrics.breakEven.breakEvenCustomers.toLocaleString(),
            ],
          ...
          <tbody>
            <tr className="border-b border-border/60">
              ...
              {(["conservative", "base", "optimistic"] as const).map((k) => (
                <td key={k} className="py-1.5 text-right tabular-nums">{formatCurrency(scenarios.scenarios[k].revenue, currency, { compact: true })}</td>
              ))}
            </tr>
            <tr>
              <td className="py-1.5 text-muted-foreground">Customers</td>
              {(["conservative", "base", "optimistic"] as const).map((k) => (
                <td key={k} className="py-1.5 text-right tabular-nums">{scenarios.scenarios[k].customers.toLocaleString()}</td>
              ))}
            </tr>
          </tbody>
        </table>
      ...
      <Section title="Strengths">
        <BulletList items={insights.strengths.map((i) => i.message)} empty="None identified." />
      </Section>
      ...
      <Section title="Recommended next experiments">
        <BulletList items={insights.recommendedActions.map((i) => i.message)} ordered />
      </Section>

      <Section title="Assumptions" last>
        <p className="text-xs text-muted-foreground">
          This report evaluates the economics of the assumptions entered on {new Date(project.createdAt).toLocaleDateString()}, last updated{" "}
          {new Date(project.updatedAt).toLocaleDateString()}. It does not predict success — it assesses whether the current numbers add up.
        </p>
      </Section>
    </div>
  );
...
function BulletList({ items, empty, ordered }: { items: string[]; empty?: string; ordered?: boolean }) {
  if (items.length === 0) return <p className="text-sm text-muted-foreground">{empty ?? "None."}</p>;
